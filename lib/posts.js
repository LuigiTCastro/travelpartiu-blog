...
function addExternalLinkAttrs(htmlContent) {
  return htmlContent.replace(
    /<a href="(https?:\/\/[^"]+)">/gi,
    (match, href) => `<a href="${href}" target="_blank" rel="noopener noreferrer sponsored">`
  );
}

function insertMidContentMarker(htmlContent) {
...
export async function getPostBySlug(slug) {
  ...
  contentHtml = insertMidContentMarker(contentHtml);
  contentHtml = addExternalLinkAttrs(contentHtml);
